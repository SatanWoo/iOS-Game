...
-(id)init{
    ...
    if(self){
        isAuto = false;
        [MusicCenter playModeBackgroundMusic:singleModeSecond];
        [self LoadImage];
        [self LoadMenu];
        [self LoadAnimation];
...
-(void)LoadImage{
    ...
    CCSprite * backgroundImage = [CCSprite spriteWithFile:OC("tc_gi_single_in_tian.png")];
...
-(void)LoadAnimation{
    ...
    [_itemLayer InitItemsWithMode:gameSingleMode Level:2]; 
...
-(void)CreateRate
{
    ...
    [_rateLayer setBackground:OC("rateFood")];
...
{ 
    ...
    [_pourLayer PourNext:gameSingleMode Level:2];
}

-(void)InsertCoin
{
    if(![_itemLayer IsFinish]) return;
    [_scoreLayer AddDebtWithLimit:10 WithPourNum:[_pourLayer GetPourNum]];
}
