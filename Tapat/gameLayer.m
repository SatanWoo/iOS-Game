...
//		[self testMice];
		...
		[self setStatu];
		readyIndex=0;
		startArray=[NSArray arrayWithObjects:[NSString stringWithFormat:@"Level %d",Level],@"3",@"2",@"1",@"Start",nil];
		[startArray retain];
//		[self newGame];
...
-(void)initData{
	...
	tempForFood = 0;
	if(Level == 1){
		miceRefreshTime = 2;		miceLiveTime = 3;
	...
	if(Level == 2){
		miceRefreshTime = 2;		miceLiveTime = 2;
        propRefreshTime = 3;        propLiveTime = 2;
		
		firstMice = 100;		secondMice = 60;				thirdMice = 30;
		blood_1 = 100;			blood_2    = 60;		    	
	...
	if(Level == 4){
		miceRefreshTime = 2;		miceLiveTime = 1;
		propRefreshTime = 3;        propLiveTime = 2;
	...
	if(Level == 6){
		miceRefreshTime = 2;		miceLiveTime = 1.5;
		propRefreshTime = 2.5;        propLiveTime = 2;
...
-(void)gameTimeRun{
	...
	[self setStatu];
	if(gameRunTime + snakeTime >= totalGameTime) [self gameOver];
}
...
-(void)snakeAction{
	snakeTime += 5.0;
    [self updateGameTime];
	if(gameRunTime + snakeTime >= totalGameTime){
		[self gameOver];
	}
}
...
-(void)updateGameTime{	
	int sec,min;
	sec = (int) (totalGameTime - gameRunTime - snakeTime);
	if(sec <=0 ) sec = 0;
	min = sec / 60; 
	sec = sec % 60;
	NSString* updateTime;
...
-(void)gameOver{
	...
	tempScore = score;
	tempWinScore = winScore;
	if(score >= winScore) [SceneManager goWin:nil];
	else [SceneManager goLose:nil];
}
